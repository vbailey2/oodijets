...
void drawhists(bool isunfold = 0)
{
  ...
  if(!isunfold)h_xj[0] = (TH2F*)fproj->Get("h_xj_0");
  else h_xj[0] = (TH2F*)fproj->Get("h_xjunfold_0");
  const int n_xj = h_xj[0]->GetNbinsX();
  const int n_final = h_xj[0]->GetNbinsY();
  for(int i = 0; i < ncent; i++)
    {
      if(isunfold) h_xj[i] = (TH2F*)fproj->Get(Form("h_xjunfold_%i",i));
      else
	{
  ...
  for(int ipt = 0; ipt < n_final; ipt++)
    {
      c->cd();
      if(!isunfold)cleg->AddEntry("",Form("%2.1f < p_{T}^{calib} < %2.1f GeV",h_xj[0]->GetYaxis()->GetBinLowEdge(ipt+1),h_xj[0]->GetYaxis()->GetBinLowEdge(ipt+2)),"");
      else cleg->AddEntry("",Form("%2.1f < p_{T} < %2.1f GeV",h_xj[0]->GetYaxis()->GetBinLowEdge(ipt+1),h_xj[0]->GetYaxis()->GetBinLowEdge(ipt+2)),"");
      for(int icent = 0; icent < ncent; icent++)
	{
	  ...
	  if(icent == 0) h_xj1D[icent][ipt]->Draw();
	  else h_xj1D[icent][ipt]->Draw("SAME");
	  ...
	  for(int icent = 0; icent < ncent; icent++)
	    {
	      ...
	      if(icent == 0) h_dphi1D[icent][ipt]->Draw();
	      else h_dphi1D[icent][ipt]->Draw("SAME");

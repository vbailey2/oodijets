...
auto FindCentBinData = [](float mbd_charge_sum) -> int 
{
   for(int i=0; i<nCentBins; i++){
      if(mbd_charge_sum < mbdcharge_boundaries[i] && mbd_charge_sum >= mbdcharge_boundaries[i+1]) return i;
   }   
   return -1; 
};
...
auto FindCentBinMC = [](float mbd_charge_sum) -> int 
{
   ...
   for(int i=0; i<nCentBins; i++){
      if(mbd_charge_sum_scaled < mbdcharge_boundaries_mc[i] && mbd_charge_sum_scaled >= mbdcharge_boundaries_mc[i+1]) return i;
   }   
   return -1; 
};
...
void getDijets(string infile = "/sphenix/tg/tg01/jets/jpark4/Run25OO/TTrees/SkimmedTrees/outree_skimjet_MC_inclusive_merged.root", string outfile = "output.root", bool ismc = true)
{
  ...
  for(int i = 0; i < nentries; i++){
    ...
    //event level cuts
    int centbin = -999;
    if(!ismc) centbin = FindCentBinData(mbdcharge);
    else centbin = FindCentBinMC(mbdcharge);
    if(centbin < 0) continue;
    int njets = 0;
    if(ismc) njets = njetsmc;
    else njets = njetsdata;

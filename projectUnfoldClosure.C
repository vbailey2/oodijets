void projectUnfoldClosure(bool ishalf = 0)
{
  ...
  {
    if (ishalf)
      h_2Dunfold[i] = (TH2F *)f->Get(Form("hUnfolded2DHalf_cent%i", i));
    else
      h_2Dunfold[i] = (TH2F *)f->Get(Form("hUnfolded2D_cent%i", i));
    h_2D_unsymunfold[i] = (TH2F *)h_2Dunfold[i]->Clone();
    h_2D_unsymunfold[i]->SetName(Form("h_2D_unsymunfold_%i", i));
    h_2D_unsymunfold[i]->Reset();

    if (ishalf)
      h_2Dtrue[i] = (TH2F *)f->Get(Form("hTrue2DB%i", i));
    else
      h_2Dtrue[i] = (TH2F *)f->Get(Form("hTrue2D%i", i));
    h_2D_unsymtrue[i] = (TH2F *)h_2Dtrue[i]->Clone();
    h_2D_unsymtrue[i]->SetName(Form("h_2D_unsymtrue_%i", i));
    h_2D_unsymtrue[i]->Reset();

    if (ishalf)
      h_2Dmeas[i] = (TH2F *)f->Get(Form("hMeas2DB%i", i));
    else
      h_2Dmeas[i] = (TH2F *)f->Get(Form("hMeas2D%i", i));
